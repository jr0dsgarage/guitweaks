...
local function ResolveTexture(tex, type)
    ...
    elseif type == "TempMaxHealthLoss" then
         -- Temp Max Health Loss (The "Purple" bar)
         if pf.PlayerFrameContent and pf.PlayerFrameContent.PlayerFrameContentMain and pf.PlayerFrameContent.PlayerFrameContentMain.HealthBarsContainer and pf.PlayerFrameContent.PlayerFrameContentMain.HealthBarsContainer.PlayerFrameTempMaxHealthLoss then
             targetFrame = pf.PlayerFrameContent.PlayerFrameContentMain.HealthBarsContainer.PlayerFrameTempMaxHealthLoss
         end
    end
    ...
    if PersonalResourceDisplayFrame then
        ...
        if PersonalResourceDisplayFrame.HealthBarsContainer then
            ...
            if loss and loss.IsObjectType and loss:IsObjectType("StatusBar") then
                -- Resolve specific texture for Loss bar (PlayerFrame uses a different texture for this than the main health bar)
                local inputLoss = addon.db.prdMatchHealth and "::BlizzPlayer::" or addon.db.prdTextureHealth
                local texLoss, isLossAtlas, sparkLoss, isSparkLossAtlas = ResolveTexture(inputLoss, "TempMaxHealthLoss")

                -- Apply immediately
                ApplyTexSimple(loss, texLoss, isLossAtlas, bgHealth, isBgHealthAtlas, colHealth, sparkLoss, isSparkLossAtlas)
                ...
                if not loss.GarageHooked then
                     hooksecurefunc(loss, "SetStatusBarTexture", function(self, tex)
                         ...
                         local input = addon.db.prdMatchHealth and "::BlizzPlayer::" or addon.db.prdTextureHealth
                         local rTex, rIsAtlas = ResolveTexture(input, "TempMaxHealthLoss")
